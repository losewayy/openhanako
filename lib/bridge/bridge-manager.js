...
export class BridgeManager {
  ...
  _createStreamDelivery({ adapter, chatId, isGroup, platform, messageThreadId, replyContext }) {
    ...
    let delivery;
    if (mode === "draft") {
      delivery = this._createDraftStreamDelivery({ adapter, chatId, capability, context });
    } else if (mode === "edit_message") {
      delivery = this._createEditMessageStreamDelivery({ adapter, chatId, capability, context });
    } else if (mode === "block") {
      delivery = this._createBlockStreamDelivery({ adapter, chatId, context });
    } else {
      delivery = this._createBatchDelivery({ adapter, chatId, context });
    }

    // 包装 finish 为幂等操作（message_end 和 _flushPending 可能都触发 finish）
    const origFinish = delivery.finish.bind(delivery);
    let finished = false;
    delivery.finish = async (text) => {
      if (finished) return;
      finished = true;
      return origFinish(text);
    };
  ...
  async _flushPending(sessionKey) {
    ...
    try {
      ...
      let reply = await this._hub.send(merged, {
        ...
        onFinish: (text) => delivery.finish(text).catch((err) => debugLog()?.warn("bridge", `delivery.finish failed: ${err.message}`)),
