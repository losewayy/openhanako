...
import { isWin32PathLike, quoteCmdArg, splitShellLikeArgs as splitShellLikeArgsBase } from "../shell/shell-utils.js";
...
function splitShellLikeArgs(command) {
  return splitShellLikeArgsBase(command, {
...
function resolvePowerShellExecutable(token, env) {
  const raw = String(token || "");
  if (isWin32PathLike(raw) || raw.includes("\\") || raw.includes("/")) return raw;
  const commandName = basenameRuntimePath(raw).toLowerCase();
  if (commandName === "pwsh" || commandName === "pwsh.exe") {
    return env?.HANA_POWERSHELL || "pwsh.exe";
  }
...
function parsePowerShellFileCommand(command, env) {
  ...
  return {
    executable: env?.HANA_POWERSHELL || "powershell.exe",
...
export function createWin32Exec({ sandbox = null } = {}) {
  return async (command, cwd, { onData, signal, timeout, env }) => {
    ...
    if (route.runner === "powershell" || route.runner === "powershell-file") {
