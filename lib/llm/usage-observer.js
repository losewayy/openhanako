...
let _onUsageCallback = null;

/**
 * 注册全局 LLM 用量回调，每次 logLlmUsage 被调用时触发。
 * 用于 Token Usage 统计等需要追踪所有 LLM 调用（聊天、频道、utility）的场景。
 * @param {Function|null} fn  (record) => void
 */
export function setOnUsage(fn) {
  _onUsageCallback = typeof fn === "function" ? fn : null;
}
...
export function logLlmUsage({
  ...
  // 全局用量回调（Token Usage 统计等）
  try {
    _onUsageCallback?.(record);
  } catch {
    // Callback must never affect model calls.
  }
